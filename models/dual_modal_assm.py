...
# Portions of this file are adapted from MambaIR's MambaIRv2 ASSM:
# https://github.com/csguoh/MambaIR/blob/main/basicsr/archs/mambairv2_arch.py
# MambaIR is licensed under the Apache License, Version 2.0.
# Adapted and substantially modified in 2025 by Qin Jiahong for dual-modal
# pansharpening. See ../THIRD_PARTY_NOTICES.md and
# ../LICENSES/Apache-2.0.txt for attribution and license information.
# Modifications Copyright (c) 2025 Qin Jiahong.
"""
用于全色锐化的双模态语义路由与门控融合模块。

本实现借鉴了 MambaIRv2 ASSM 的语义路由结构，但不包含 Mamba
状态空间递推或 Selective Scan 算子。为兼容现有调用方，保留
``DualModalSelectiveScan`` 和 ``DualModal_ASSM`` 等历史名称。
"""
...
class DualModalSelectiveScan(nn.Module):
    """兼容命名的双模态门控融合块；并非 Selective Scan 状态空间算子。"""
